...
RULES: list[Rule] = [
    ...
    "Potential SQL Injection from dynamically built query",
    "high",
    re.compile(r"""(?i)(select|insert|update|delete|create|drop)\s+.*(from|into|table|values)\s+.*(\+|=|\s*format|\s*%\s*s)"""),
    ()
)
