...
RULES: list[Rule] = [
    ...
    "sql-injection-php-get-id",
    "SQL injection via 'ID' parameter in PHP (common pattern for vulnerable web apps)",
    "high",
    re.compile(r"""\b(?i)ID\s*=\s*\$_GET\['ID'\][^;]*\b(query|execute)\s*\([^)]*\$_GET\['ID'\]"""),
    (".php",),
)
