...
RULES: list[Rule] = [
    ...
    "sql-injection-parameterized-template",
    "SQL query built with string templating that might be vulnerable to injection (e.g., using Python f-strings directly in queries)",
    "high",
    re.compile(
        r"""(?i)(execute|cursor\.execute|query)\s*\(\s*f['"].*?\s*\{\s*(?!f(?:["']|\s*\.\w+\s*\(.*\)))\s*[\w\.]+\s*\}"""
    ),
    (".py",),
),
