...
RULES: list[Rule] = [
    ...
    "jwt-unverified-alg-header",
    "JWT verifier uses unverified 'alg' header to determine algorithm, enabling algorithm confusion attacks",
    "high",
    re.compile(r"""\.decode\(\s*token,\s*.*?,?\s*algorithms\s*=\s*\[?\s*header_data\[['"]alg['"]\]\s*\]?"""),
    (".py",),
)
